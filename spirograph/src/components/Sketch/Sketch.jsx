...
const chains = [
  ...
  },
  {
    ...
    blockExplorerUrl: "https://polkadot.js.org/apps/?rpc=ws%3A%2F%2Flocalhost%3A9944#/explorer",
  },
];
...
const Sketch = ({ firstName, lastName, birth, isSubmitted, onEnd, onProgress }) => {
  ...
  useEffect(() => {
    ...
    const sketch = (p) => {
      ...
      p.draw = () => {
        ...
        drawSpirograph(p, R2, r2, currentRotation, getRandomColor(p), firstName);
      ...
      const handleSaveImage = async (p) => {
        p.canvas.toBlob(async (blob) => {
          ...
          try {
            const response = await fetch('http://localhost:5001/upload-to-ipfs', {
              method: 'POST',
      ...
      <div className="nft">
        {isEnd && (
          <div className="buttons">
            ...
            <button className="button3" onClick={() => navigate('/aiCorrection', { state: { url } })}>
              <span className="button-text">
                AI 수정하기
              </span>
              <div className="tooltip">
