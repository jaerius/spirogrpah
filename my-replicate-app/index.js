...
const pinJSONToIPFS = async (json) => {
  const data = JSON.stringify({
    ...
    pinataMetadata: { name: "testing", keyvalues: { customKey: "customValue", customKey2: "customValue2" } },
    pinataContent: json,
  });
...
async function runReplicateModel(url, prompt) {
  try {
    ...
    const input = {
      width: 768, height: 768, image: url, prompt: prompt, refine: 'expert_ensemble_refiner', scheduler: 'K_EULER',
      lora_scale: 0.6, num_outputs: 1, guidance_scale: 7.5, apply_watermark: false, high_noise_frac: 0.8,
      negative_prompt: '', prompt_strength: 0.8, num_inference_steps: 25,
    };
...
app.post('/save-url', async (req, res) => {
  ...
  try {
    ...
    res.json({ finalIpfsUrl: finalIpfsUrl, ipfsUrl: `https://gateway.pinata.cloud/ipfs/${pinataResponse.data.IpfsHash}`, metadataResult: `https://gateway.pinata.cloud/ipfs/${metadataResult.IpfsHash}` });
